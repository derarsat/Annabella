...
<h1 class="admin-title">Edit Gallery Image</h1>
<form method="post" action="{{route('gallery-images.update',$gallery_image->id)}}" class="admin-section max-w-xl" enctype="multipart/form-data">
    @csrf @method("put")
    <div class="flex items-center gap-6">
        ...
        <img class="w-32 " src="{{ @App::make('url')->to('/') . '/storage' . $gallery_image->url}}" alt="">
    </div>
    ...
    <div class="flex items-center gap-6">
        ...
        <img src="{{ @App::make('url')->to('/') . '/storage' . $gallery_image->featured_image}}" class="w-32" alt="">
    </div>
    <div class="input-group">
        <label for="grid">Number Of Cols </label>
        <input type="number" value="{{$gallery_image->grid}}" name="grid">
        <span>From a grid of 12 columns, how many columns this image will expand</span> @error('grid')
        <span class="error-label" role="alert">
            <strong>{{ $message }}</strong>
        </span> @enderror
    </div>
